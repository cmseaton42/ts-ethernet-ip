 ...
 */
export const build = (port: number, link: number | string): Buffer => {
    if (port <= 0) throw new Error('Port Number must be a Positive Integer');
    if (typeof link === 'number' && link < 0) throw new Error('Link Number must be a Positive Integer or String');

    let buf: Buffer | null = null;
    let portIdentifierByte = PORT_SEGMENT; // Set High Byte of Segement (0x00)

    // Check Link Buffer Length
    let linkBuf: Buffer = Buffer.from([]);
