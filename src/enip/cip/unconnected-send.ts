 ...
 */
export const build = (messagRequest: Buffer, path: Buffer, timeout: number = 2000): Buffer => {
    ...
    const msgReqLen = messagRequest.length;
    ...
    if (msgReqLen % 2 === 1) {
        ...
        buf = Buffer.concat([buf, msgReqLenBuf, messagRequest, nullBuf, pathLenBuf, nullBuf, path]);
    } else {
        buf = Buffer.concat([buf, msgReqLenBuf, messagRequest, pathLenBuf, nullBuf, path]);
    }
